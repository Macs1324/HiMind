...
async function findExpertsByQuery(query: string, organizationId: string): Promise<ExpertMatch[]> {
  // Use the knowledge engine to search for relevant knowledge points
  const searchResults = await getKnowledgeEngine().searchKnowledge(query, organizationId);
  ...
  const experts = Array.from(expertMap.values()).map(expert => ({
    ...expert,
    expertiseScore: (expert.relevantContributions / Math.max(expert.totalContributions, 1)) * 
...
async function findPotentialAnswers(query: string, organizationId: string): Promise<KnowledgeAnswer[]> {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Find experts and potential answers in parallel
    const [experts, potentialAnswers] = await Promise.all([
      findExpertsByQuery(query, org.id),
      findPotentialAnswers(query, org.id)
    ]);
    ...
    const result: ExpertSearchResult = {
      ...
      potentialAnswers,
      hasDirectAnswers: potentialAnswers.length > 0 && potentialAnswers[0].relevanceScore > 0.8
    };

    console.log(`👥 [KNOWLEDGE SEARCH] Found ${experts.length} experts, ${potentialAnswers.length} potential answers`);
