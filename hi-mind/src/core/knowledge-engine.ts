...
export class KnowledgeEngine {
  ...
  constructor() {
    ...
    try {
      ...
      throw new Error(
        "Service role key required for Knowledge Engine. Please set SUPABASE_SERVICE_ROLE_KEY environment variable.",
      );
    }
    ...
    if (!process.env.OPENAI_API_KEY) {
      throw new Error(
        "OpenAI API key required for Knowledge Engine. Please set OPENAI_API_KEY environment variable.",
      );
    }
  ...
  async ingestKnowledgeSource(
    ...
    console.log(
      `🧠 [KNOWLEDGE ENGINE] Ingesting ${source.platform}:${source.sourceType} - ${source.externalId}`,
    );
    ...
    if (source.authorExternalId && source.platform) {
      authorPersonId = await this.getOrCreateAuthorPerson(
        source,
        organizationId,
      );
    }
    ...
    this.updateTopicClustersAsync(
      organizationId,
      knowledgePoint.id,
      processed.embedding,
    );
    this.updateExpertiseScoresAsync(
      organizationId,
    ...
    console.log(
      `✅ [KNOWLEDGE ENGINE] Processed knowledge point: ${knowledgePoint.id}`,
    );
  ...
  async ingestSlackMessageWithContext(
    ...
    console.log(
      `🧠 [CONTEXTUAL INGEST] Processing Slack message: ${source.externalId}`,
    );
    ...
    if (!contextualResult.shouldIndex) {
      console.log(
        `⏭️ [CONTEXTUAL INGEST] Skipping non-substantial message: ${source.externalId}`,
      );
      return null;
    }
    ...
    if (sourceError) {
      console.error(
        "❌ [CONTEXTUAL INGEST] Failed to store knowledge source:",
        sourceError,
      );
      throw sourceError;
    }
    ...
    const shouldFilter =
      !isThread &&
      (this.isObviousQuestion(source.content) ||
        this.isObviousQuestion(contextualResult.contextualContent));

    if (shouldFilter) {
      console.log(
        `⚠️ [CONTEXTUAL INGEST] Safety filter caught question that LLM missed: ${source.externalId}`,
      );
      console.log(`🔍 [CONTEXTUAL INGEST] Original: "${source.content}"`);
      console.log(
        `🔍 [CONTEXTUAL INGEST] Enhanced: "${contextualResult.contextualContent}"`,
      );
      return null;
    }
    ...
    if (isThread) {
      console.log(
        `🧵 [CONTEXTUAL INGEST] Processing thread reply with relaxed filtering: "${source.content}"`,
      );
    }
    ...
    if (pointError) {
      console.error(
        "❌ [CONTEXTUAL INGEST] Failed to store knowledge point:",
        pointError,
      );
      throw pointError;
    }
    ...
    console.log(
      `✅ [CONTEXTUAL INGEST] Processed contextual knowledge point: ${knowledgePoint.id}`,
    );
    console.log(
      `🎯 [CONTEXTUAL INGEST] Enhanced: "${source.content}" → "${contextualResult.contextualContent}"`,
    );
   ...
   */
  private async getOrCreateAuthorPerson(
    source: KnowledgeSource,
    organizationId: string,
  ): Promise<string | null> {
    if (!source.authorExternalId || !source.platform) {
    ...
    try {
      console.log(
        `🔍 [KNOWLEDGE ENGINE] Resolving author for ${source.platform}:${source.authorExternalId}`,
      );
      ...
      if (existingIdentity) {
        console.log(
          `✅ [KNOWLEDGE ENGINE] Author already linked to person: ${existingIdentity.people?.display_name}`,
        );
      ...
      if (source.platform === "slack") {
        const slackInfo = await this.fetchSlackUserInfo(
          source.authorExternalId,
        );
        ...
        const githubInfo = await this.fetchGithubUserInfo(
          source.authorExternalId,
        );
      ...
      if (!userEmail) {
        console.log(
          `⚠️ [KNOWLEDGE ENGINE] No email found for ${source.platform}:${source.authorExternalId}. Creating person without email for manual linking later.`,
        );

        // Try to find existing person by fuzzy name matching before creating new one
        if (displayName) {
          const peopleService = new (
            await import("@/lib/database")
          ).PeopleService(this.supabase);

          // Check for existing person with similar name
          const existingPerson = await this.findPersonByFuzzyNameMatch(
            displayName,
            organizationId,
          );
          ...
          if (existingPerson) {
            console.log(
              `🔗 [KNOWLEDGE ENGINE] Found existing person by name match: "${displayName}" → "${existingPerson.display_name}"`,
            );
            ...
            console.log(
              `✅ [KNOWLEDGE ENGINE] Linked ${source.platform} identity to existing person (fuzzy name match)`,
            );
            ...
            if (newPerson) {
              console.log(
                `👤 [KNOWLEDGE ENGINE] Created new person: ${newPerson.display_name}`,
              );
              ...
              console.log(
                `🔗 [KNOWLEDGE ENGINE] Linked ${source.platform} identity to new person`,
              );
              return newPerson.id;
            }
          }
      ...
      const peopleService = new (await import("@/lib/database")).PeopleService(
        this.supabase,
      );
      const { data: existingPerson } = await peopleService.getPersonByEmail(
        userEmail,
        organizationId,
      );
      ...
      if (existingPerson) {
        ...
        console.log(
          `🔗 [KNOWLEDGE ENGINE] Linking ${source.platform} identity to existing person: ${existingPerson.display_name} (${userEmail})`,
        );
      } else {
        // Create new person
        const { data: newPerson } = await peopleService.createPerson(
        ...
        if (!newPerson) {
          console.error(
            `❌ [KNOWLEDGE ENGINE] Failed to create person for ${userEmail}`,
          );
          return null;
        }
        ...
        console.log(
          `👤 [KNOWLEDGE ENGINE] Created new person: ${newPerson.display_name} (${userEmail})`,
        );
      }
      ...
      console.log(
        `✅ [KNOWLEDGE ENGINE] Successfully linked ${source.platform}:${source.authorExternalId} to person`,
      );
      return personId;
    } catch (error) {
      console.error(
        `❌ [KNOWLEDGE ENGINE] Failed to resolve author by email:`,
        error,
      );
   ...
   */
  private async fetchSlackUserInfo(userId: string): Promise<{
    email?: string;
    displayName?: string;
    username?: string;
  } | null> {
    try {
      ...
      if (result.ok && result.user) {
        return {
          ...
          displayName:
            result.user.profile?.display_name || result.user.profile?.real_name,
          username: result.user.name,
        };
      }
    } catch (error) {
      console.error(
        `❌ [KNOWLEDGE ENGINE] Failed to fetch Slack user info for ${userId}:`,
        error,
      );
    }
    return null;
   ...
   */
  private async fetchGithubUserInfo(userId: string): Promise<{
    email?: string;
    displayName?: string;
    username?: string;
  } | null> {
    try {
      console.log(
        `🔍 [KNOWLEDGE ENGINE] Fetching GitHub user info for: ${userId}`,
      );
      ...
      if (!process.env.GITHUB_TOKEN) {
        console.log(
          `⚠️ [KNOWLEDGE ENGINE] GITHUB_TOKEN not set, cannot fetch user info`,
        );
        return null;
      }
      ...
      console.error(
        `❌ [KNOWLEDGE ENGINE] Failed to fetch GitHub user info for ${userId}:`,
        error,
      );
    }
    return null;
   ...
   */
  private async findPersonByFuzzyNameMatch(
    targetName: string,
    organizationId: string,
  ): Promise<any | null> {
    try {
      ...
      const fuzzyMatches = people
        .map((person: any) => ({
          ...
          similarity: this.calculateNameSimilarity(
            normalizedTarget,
            this.normalizeName(person.display_name),
          ),
        }))
  ...
  async searchKnowledge(
    ...
    console.log("Generating embedding, is this shit gonna hang?");
    ...
    console.log("Calling supabase function, is this shit gonna hang?");
    const { data: rawKnowledgeMatches } = await this.supabase.rpc(
      "find_similar_knowledge",
      {
        query_embedding: `[${queryEmbedding.join(",")}]`,
        org_id: organizationId,
        similarity_threshold: 0.1,
        result_limit: 25, // Get many candidates for LLM to choose from
      },
    );

    // 4. Use LLM to intelligently rank and select top 3 most relevant results
    console.log("Ranking results, is this shit gonna hang?");
    const rankedKnowledgeMatches = await this.rankResultsWithLLM(
      query,
      rawKnowledgeMatches || [],
    );

    // 5. Find relevant topics and their experts
    const topicMatches = await this.findRelevantTopics(
      queryEmbedding,
      organizationId,
    );
    ...
    console.log(
      `📊 [KNOWLEDGE ENGINE] Found ${rawKnowledgeMatches?.length || 0} initial matches, LLM selected ${rankedKnowledgeMatches.length} top results, ${result.suggestedExperts.length} expert suggestions`,
    );
    return result;
  }
   ...
   */
  private async rankResultsWithLLM(
    query: string,
    candidates: any[],
    ...
    try {
      console.log(
        `🤖 [KNOWLEDGE ENGINE] LLM ranking ${candidates.length} candidates for query: "${query}"`,
      );
...
Your selection (numbers only):`;
      ...
      const openAiResponse = this.openai.chat.completions.create({
        model: "gpt-3.5-turbo",
      ...
      const defaultResponse = new Promise((res) =>
        setTimeout(
          () => res({ choices: [{ message: { content: "1,2,3" } }] }),
          5000,
        ),
      );
      ...
      if (!selection) {
        console.log(
          `⚠️ [KNOWLEDGE ENGINE] LLM ranking failed, falling back to similarity order`,
        );
      ...
      if (selectedIndices.length >= 1) {
        ...
        console.log(
          `✅ [KNOWLEDGE ENGINE] LLM selected ${rankedResults.length} results: [${selectedIndices.map((i) => i + 1).join(", ")}]`,
        );
        return rankedResults;
      } else {
        console.log(
          `⚠️ [KNOWLEDGE ENGINE] Invalid LLM selection "${selection}", falling back to similarity order`,
        );
        return candidates.slice(0, 3);
      }
    } catch (error) {
  ...
  async discoverTopicClusters(
    ...
    console.log(
      `🎯 [KNOWLEDGE ENGINE] Discovering topic clusters for org: ${organizationId}`,
    );
    ...
    if (!knowledgePoints || knowledgePoints.length < minClusterSize) {
      console.log(
        `⚠️ [KNOWLEDGE ENGINE] Not enough knowledge points for clustering (${knowledgePoints?.length || 0} < ${minClusterSize})`,
      );
      return {
        topics: [],
        stats: {
    ...
    console.log(
      `📊 [KNOWLEDGE ENGINE] Clustering ${knowledgePoints.length} knowledge points...`,
    );
    console.log(`🔍 [KNOWLEDGE ENGINE] Sample knowledge point structure:`, {
      ...
      embeddingLength:
        typeof knowledgePoints[0]?.embedding === "string"
          ? knowledgePoints[0]?.embedding?.length
          : "N/A",
    ...
    try {
      console.log(
        `🎯 [KNOWLEDGE ENGINE] About to call performEmbeddingClustering...`,
      );
      ...
      console.log(
        `✅ [KNOWLEDGE ENGINE] performEmbeddingClustering completed successfully`,
      );
    } catch (error) {
      console.error(
        `❌ [KNOWLEDGE ENGINE] performEmbeddingClustering failed:`,
        error,
      );
      return {
        topics: [],
    ...
    const results = await this.manageTopicsIntelligently(
      organizationId,
      clusters,
    );

    console.log(
      `✨ [KNOWLEDGE ENGINE] Topic discovery complete: ${results.newTopics} new, ${results.updatedTopics} updated`,
    );
  ...
  private async performEmbeddingClustering(
    ...
    console.log(
      `🚀 [KNOWLEDGE ENGINE] Starting performEmbeddingClustering with ${knowledgePoints.length} knowledge points`,
    );
    const { minClusterSize, maxClusters } = options;

    // Convert embeddings to arrays of numbers
    const embeddings = knowledgePoints
      .map((kp) => {
        try {
          let embedding;
          if (typeof kp.embedding === "string") {
            ...
            console.log(
              `⚠️ [KNOWLEDGE ENGINE] Unknown embedding format for ${kp.id}:`,
              typeof kp.embedding,
            );
            return [];
          }
          ...
          if (Array.isArray(embedding) && embedding.length === 1536) {
            ...
            console.log(
              `⚠️ [KNOWLEDGE ENGINE] Invalid embedding dimensions for ${kp.id}: ${embedding?.length}`,
            );
            return [];
          }
        } catch (error) {
          console.error(
            `❌ [KNOWLEDGE ENGINE] Failed to parse embedding for ${kp.id}:`,
            error,
          );
          return [];
        }
    ...
    console.log(
      `✅ [KNOWLEDGE ENGINE] Successfully parsed ${embeddings.length} valid embeddings`,
    );

    // Determine optimal number of clusters with content-aware logic
    const optimalK = this.determineOptimalClusters(
      embeddings.length,
      maxClusters,
      knowledgePoints,
    );
    ...
    console.log(
      `🎯 [KNOWLEDGE ENGINE] Using ${optimalK} clusters for ${embeddings.length} points`,
    );
    ...
    console.log(
      `🎯 [KNOWLEDGE ENGINE] Raw clusters from K-means:`,
      clusters.map((c) => c.length),
    );

    // Map back to knowledge points and filter by minimum size
    const clustersWithPoints = clusters
      .map((clusterIndices, clusterIndex) => {
        console.log(
          `🔍 [KNOWLEDGE ENGINE] Cluster ${clusterIndex}: ${clusterIndices.length} indices: [${clusterIndices.slice(0, 5).join(", ")}...]`,
        );
        ...
        const points = clusterIndices
          .map((index) => {
            if (index >= 0 && index < knowledgePoints.length) {
              ...
              console.log(
                `⚠️ [KNOWLEDGE ENGINE] Invalid index ${index} for ${knowledgePoints.length} knowledge points`,
              );
              return null;
            }
          })
        ...
        console.log(
          `📊 [KNOWLEDGE ENGINE] Cluster ${clusterIndex}: ${clusterIndices.length} indices → ${points.length} valid points`,
        );

        if (points.length < minClusterSize) {
          console.log(
            `❌ [KNOWLEDGE ENGINE] Cluster ${clusterIndex} filtered out: ${points.length} < ${minClusterSize}`,
          );
          return null;
        }
        ...
        console.log(
          `✅ [KNOWLEDGE ENGINE] Cluster ${clusterIndex}: keeping ${points.length} points`,
        );
    ...
    console.log(
      `📊 [KNOWLEDGE ENGINE] Filtered to ${clustersWithPoints.length} clusters with min size ${minClusterSize}`,
    );
  ...
  private determineOptimalClusters(
    ...
    if (knowledgePoints) {
      const platforms = new Set(
        knowledgePoints.map((kp) => kp.knowledge_sources?.platform),
      );
      const sourceTypes = new Set(
        knowledgePoints.map((kp) => kp.knowledge_sources?.source_type),
      );
      ...
      console.log(
        `🎯 [KNOWLEDGE ENGINE] Platform diversity: ${platforms.size} platforms, ${sourceTypes.size} source types`,
      );
    }
  ...
  private kMeansClustering(
    ...
    while (!converged && iteration < maxIterations) {
      ...
      converged = assignments.every(
        (assignment, index) => assignment === newAssignments[index],
      );
    ...
    console.log(
      `🎯 [KNOWLEDGE ENGINE] K-means complete: ${clusters.length} raw clusters → ${filteredClusters.length} non-empty clusters`,
    );
  ...
  private initializeCentroidsKMeansPlusPlus(
    ...
    console.log(
      `🎯 [KNOWLEDGE ENGINE] Initialized ${k} centroids using K-means++`,
    );
    return centroids;
  }
   ...
   */
  private async manageTopicsIntelligently(
    organizationId: string,
    newClusters: any[],
  ): Promise<{
    topics: any[];
    ...
    console.log(
      `🧠 [KNOWLEDGE ENGINE] Starting intelligent topic management for ${newClusters.length} clusters`,
    );
    ...
    console.log(
      `📊 [KNOWLEDGE ENGINE] Found ${topics.length} existing topics to compare against`,
    );
    ...
    const existingCentroids = topics
      .map((topic: any) => ({
        topic,
    ...
    for (const cluster of newClusters) {
      ...
      if (bestMatch) {
        ...
        console.log(
          `🔄 [KNOWLEDGE ENGINE] Updating existing topic "${bestMatch.topic.name}" (similarity: ${(bestSimilarity * 100).toFixed(1)}%)`,
        );

        const updatedTopic = await this.updateExistingTopic(
          bestMatch.topic,
          cluster,
        );
        if (updatedTopic) {
        ...
        // Create new topic
        console.log(
          `✨ [KNOWLEDGE ENGINE] Creating new topic for unmatched cluster`,
        );

        const newTopic = await this.createNewTopicFromCluster(
          organizationId,
          cluster,
        );
        if (newTopic) {
    ...
    const unmatchedTopics = topics.filter(
      (topic: any) => !usedTopicIds.has(topic.id),
    );
    for (const topic of unmatchedTopics) {
      console.log(
        `🗄️ [KNOWLEDGE ENGINE] Archiving outdated topic: "${topic.name}"`,
      );
    ...
    console.log(
      `🎯 [KNOWLEDGE ENGINE] Topic management complete: ${results.newTopics} new, ${results.updatedTopics} updated, ${results.archivedTopics} archived`,
    );
    return results;
  }
   ...
   */
  private async createOrUpdateTopicFromCluster(
    organizationId: string,
    cluster: any,
  ): Promise<any | null> {
    try {
      ...
      if (!topicName) {
        console.log(
          "⚠️ [KNOWLEDGE ENGINE] Failed to generate topic name for cluster",
        );
        return null;
      }

      // Check if topic already exists (fuzzy matching)
      const existingTopic = await this.findSimilarTopic(
        topicName,
        organizationId,
      );
      ...
      if (existingTopic) {
        ...
        topic = updatedTopic;
        console.log(
          `📝 [KNOWLEDGE ENGINE] Updated existing topic: ${topicName}`,
        );
      } else {
        // Create new topic
      ...
      await this.linkKnowledgePointsToTopic(
        topic.id,
        cluster.points.map((p: any) => p.id),
      );
      ...
      console.error(
        "❌ [KNOWLEDGE ENGINE] Failed to create/update topic from cluster:",
        error,
      );
      return null;
    }
   ...
   */
  private async generateTopicNameFromCluster(
    cluster: any,
  ): Promise<string | null> {
    try {
      ...
      if (topicName && topicName.length > 0) {
        console.log(
          `🎯 [KNOWLEDGE ENGINE] Generated topic name: "${topicName}" for cluster of ${cluster.size} points`,
        );
        return topicName;
      }
    } catch (error) {
  ...
  private async extractClusterThemes(cluster: any): Promise<string> {
    try {
      ...
      const allKeywords = cluster.points.flatMap(
        (point: any) => point.keywords || [],
      );
...
Respond with just the key themes (not a topic name), separated by semicolons:`;
   ...
   */
  private async generateTopicNameFromThemes(
    themes: string,
    cluster: any,
  ): Promise<string | null> {
...
Topic Name:`;
      ...
      console.error(
        "❌ [KNOWLEDGE ENGINE] Topic name generation failed:",
        error,
      );
      return null;
    }
  ...
  private getClusterStats(cluster: any): string {
    const platforms = cluster.points
      .map((p: any) => p.knowledge_sources?.platform)
      .filter(Boolean);
    const sourceTypes = cluster.points
      .map((p: any) => p.knowledge_sources?.source_type)
      .filter(Boolean);
    ...
    const sourceCounts = sourceTypes.reduce(
  ...
  private async findSimilarTopic(
    ...
    for (const topic of topics) {
      ...
      if (similarity > 0.8) {
        // High threshold for topic merging
        return topic;
      }
    }
   ...
   */
  async processSlackMessageWithContext(
    source: KnowledgeSource & {
      channelId?: string;
      threadTs?: string;
      timestamp: string;
    ...
    try {
      console.log(
        `🧠 [CONTEXTUAL] Processing message with context: ${source.externalId}`,
      );
      ...
      if (!enhancement.shouldIndex) {
        console.log(
          `⏭️ [CONTEXTUAL] Skipping message (not substantial): ${source.externalId}`,
        );
        return {
      ...
      console.log(
        `✅ [CONTEXTUAL] Enhanced message: "${source.content}" → "${enhancement.contextualContent}"`,
      );
      if (enhancement.relatedQuestions.length > 0) {
        console.log(
          `🔗 [CONTEXTUAL] Found related questions: ${enhancement.relatedQuestions.join(", ")}`,
        );
      }
      ...
      console.error(
        `❌ [CONTEXTUAL] Context enhancement failed for ${source.externalId}:`,
        error,
      );
      console.log(
        `🔧 [CONTEXTUAL] Falling back to safety filter only for: "${source.content}"`,
      );

      // Apply safety filter even in fallback
      if (this.isObviousQuestion(source.content)) {
        console.log(
          `⚠️ [CONTEXTUAL] Safety filter caught question in fallback: ${source.externalId}`,
        );
        return {
          contextualContent: source.content,
          shouldIndex: false,
  ...
  private async gatherConversationContext(
    ...
    try {
      ...
      if (channelId) {
        const { data: channelContext } = await this.supabase.rpc(
          "get_channel_context",
          {
        ...
        if (channelContext) {
          ...
          context.sourceIds.push(
            ...channelContext.map((msg: any) => msg.external_id),
          );
        }
      }
      ...
      if (threadTs && channelId) {
        const { data: threadContext } = await this.supabase.rpc(
          "get_thread_context",
          {
        ...
        if (threadContext) {
          ...
          context.sourceIds.push(
            ...threadContext.map((msg: any) => msg.external_id),
          );
        }
      }
    } catch (error) {
      console.error(
        "❌ [CONTEXTUAL] Failed to gather conversation context:",
        error,
      );
    }
...
Respond with either the JSON or "SKIP":`;
      ...
      try {
        ...
        console.warn(
          "❌ [CONTEXTUAL] Failed to parse JSON response, using as-is:",
          parseError,
        );
      }
  ...
  private async processContentWithAI(
    ...
    const contextualSummary = contextualContent
      ? await this.generateSummary(contextualContent)
      : undefined;
  ...
  private async updateTopicClustersAsync(
    organizationId: string,
    knowledgePointId: string,
    embedding: number[],
  ): Promise<void> {
      ...
      try {
        ...
        for (const topic of topics || []) {
          if (topic.cluster_centroid) {
            const similarity = this.cosineSimilarity(
              embedding,
              topic.cluster_centroid,
            );
            if (similarity > 0.7) {
              // Add this knowledge point to the topic
              await this.supabase.from("knowledge_topic_memberships").upsert({
  ...
  private async updateExpertiseScoresAsync(
    organizationId: string,
    authorExternalId: string,
    knowledgePointId: string,
  ): Promise<void> {
    ...
    setTimeout(async () => {
      try {
        ...
        for (const membership of topicMemberships || []) {
          await this.supabase.from("topic_experts").upsert(
            {
  ...
  private async findRelevantTopics(
    ...
    return topics
      .map((topic: any) => ({
        ...topic,
        similarity: topic.cluster_centroid
          ? this.cosineSimilarity(queryEmbedding, topic.cluster_centroid)
          : 0,
      }))
  ...
  private async findTopicExperts(
    topics: any[],
    limit: number,
  ): Promise<ExpertMatch[]> {
    ...
    const { data: experts } = await this.supabase.rpc("find_topic_experts", {
  ...
  private async createOrUpdateTopic(
    organizationId: string,
    cluster: any,
  ): Promise<void> {
    await this.supabase.from("discovered_topics").upsert(
      {
        organization_id: organizationId,
        name: cluster.name,
  ...
  private generateTopicName(summary: string, keywords: string[]): string {
    ...
    if (keywords.length > 0) {
      ...
      return (
        primaryKeyword.charAt(0).toUpperCase() +
        primaryKeyword.slice(1) +
        " Development"
      );
    }
   ...
   */
  private async createNewTopicFromCluster(
    organizationId: string,
    cluster: any,
  ): Promise<any | null> {
    try {
  ...
  private async archiveOutdatedTopic(topicId: string): Promise<void> {
    try {
      ...
      await this.supabase.from("discovered_topics").delete().eq("id", topicId);
   ...
   */
  private async linkKnowledgePointsToTopic(
    topicId: string,
    ...
    try {
      console.log(
        `🔗 [KNOWLEDGE ENGINE] Linking ${knowledgePointIds.length} knowledge points to topic ${topicId}`,
      );
      ...
      if (error) {
        console.error(
          "❌ [KNOWLEDGE ENGINE] Failed to create topic memberships:",
          error,
        );
      } else {
        console.log(
          `✅ [KNOWLEDGE ENGINE] Successfully linked ${knowledgePointIds.length} knowledge points to topic`,
        );
      }
    } catch (error) {
      console.error(
        "❌ [KNOWLEDGE ENGINE] Error linking knowledge points to topic:",
        error,
      );
    }
  }
